...
export const list_element_class = "group flex gap-2 rounded-md transition duration-300 ease-in-out hover:bg-white hover:text-darkgrey border-1 hover:shadow-md shadow-darkgrey hover:border-darkgrey overflow-hidden cursor-pointer"
...
const activeRoomStyle = "text-darkgrey border-darkgrey "
const nonActiveRoomStyle = "text-white bg-darkgrey border-darkgrey "
export default function Rooms_list({rooms} : {rooms: Rooms_type[]}) {
    const currentRoomId = useLocation().pathname.split("/")[3]
        ...
        <>
            <ul className="flex flex-col gap-1">
                <input type="search" className={search_inp_class} placeholder="search for room.." />
                {
                    rooms.map((room) => {
                        return (
                            <Link to={"room/" + room.id} className={`${list_element_class} ${currentRoomId == room.id ? activeRoomStyle : nonActiveRoomStyle}`} key={room.id}>
